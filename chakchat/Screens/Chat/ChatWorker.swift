...
/// но как определить что сообщение первое?
/// посмотреть в coreData по memberID текущий чат с пользователем
/// окей я посмотрел и оно действительно первое, но поиск чата по memberID занимает
/// O(n), где n - количество сохраненных чатов.
/// появляется другой вопрос как делать эту проверку на первое сообщение только 1 раз
/// 
